...
document.addEventListener('DOMContentLoaded', function () {
  ...
  let startLongTask = function (task_id, current_percent = -1) {
    jQuery.ajax({
      type: 'POST',
      ...
      success: function (data) {
        if (data.status_progress !== undefined) {
          if (data.status_progress === 'next' && data.current_progress !== undefined) {
            startLongTask(task_id, data.current_progress);
            console.log('next long progress', data);
          }

          if (data.status_progress === 'end') {
            console.log('end long progress');
        ...
        // console.log('start_long_task_finished_tasks_id', task_id);
  ...
  let monitorProgress = function (task_id) {
    jQuery.ajax({
      ...
      success: function (data) {
        console.log('monitor progress', data);
